...
```bash npm2yarn
npm install @moetnavss/ui-preset --save-dev
```
...
module.exports = {
  ...
  content: [
    // ...
    "./node_modules/@moetnavss/ui/dist/**/*.{js,jsx,ts,tsx}",
  ],
  // ...
}
```

If your project is in a monorepo, you'll need to resolve the path to the `@moetnavss/ui` package from the monorepo root:
...
```css
@import "tailwindcss";
@source "../node_modules/@moetnavss/ui";
@config "@moetnavss/ui-preset";
```
...
Medusa's design system packages, including `@moetnavss/ui` and `@moetnavss/ui-preset`, are versioned independently from other `@moetnavss/*` packages. However, they're still released as part of Medusa's releases.
