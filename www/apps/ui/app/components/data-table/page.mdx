...
You import the `DataTable` component from `@moetnavss/ui`.
...
The `DataTable` component expects a table instance created using the `useDataTable` hook. Import that hook from `@moetnavss/ui`:
...
First, import the `DataTablePaginationState` type from `@moetnavss/ui`:
...
First, add the following imports from the `@moetnavss/ui` package:
...
First, add the following imports from `@moetnavss/ui`:
