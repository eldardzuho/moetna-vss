...
This doesn't apply to the design-system packages, including `@moetnavss/ui`, `@moetnavss/ui-presets`, and `@moetnavss/ui-icons`. These packages are versioned independently. However, you don't need to install and manage them separately in your Medusa application, as they are included in the `@moetnavss/admin-sdk`. If you're using them in a standalone project, such as a storefront or custom admin dashboard, refer to [this section in the Medusa UI documentation](!ui!/installation/standalone-project#updating-ui-packages) for update instructions.
...
Then, to update your Medusa application, bump the version of all `@moetnavss/*` dependencies in your `package.json`. Then, re-install dependencies:
...
Then, revert the update by changing the version of all `@moetnavss/*` dependencies in your `package.json` to the previous version and re-installing dependencies:
...
If you have a Medusa plugin project, you only need to update its `@moetnavss/*` dependencies in the `package.json` file to the latest version. Then, re-install dependencies:
