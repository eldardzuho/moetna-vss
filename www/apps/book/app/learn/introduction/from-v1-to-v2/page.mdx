...
- You need to install the following Medusa packages (All these packages use the same version):
    ...
    - `@moetnavss/framework`
    - `@moetnavss/medusa`
    - `@moetnavss/test-utils` (as a dev dependency)
...
import { loadEnv, defineConfig } from "@moetnavss/framework/utils"
...
The Medusa CLI for v2 is now in the `@moetnavss/cli` package. However, you don't need to install it globally. You can just use `npx medusa`, `yarn medusa`, or `pnpm medusa` in your Medusa projects.
...
```ts title="src/modules/brand/service.ts"
import { InferTypeOf, DAL } from "@moetnavss/framework/types"
...
In Medusa v1, you imported all Workflows SDK functions and types from the `@moetnavss/workflows-sdk` package.

In Medusa v2, you import them from the `@moetnavss/framework/workflows-sdk` package. For example:
...
In Medusa v1, you imported API-route related types and functions from the `@moetnavss/medusa` package.

In Medusa v2, you import them from the `@moetnavss/framework/http` package. For example:
...
```ts title="src/api/middlewares.ts"
import { defineMiddlewares } from "@moetnavss/framework/http"
...
In Medusa v1, you imported subscriber types from the `@moetnavss/medusa` package.

In Medusa v2, you import them from the `@moetnavss/framework` package. For example:
...
import { ChatBubbleLeftRight } from "@moetnavss/icons"
