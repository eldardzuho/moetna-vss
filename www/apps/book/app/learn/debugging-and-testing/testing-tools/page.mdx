...
To use the Testing Framework, install `@moetnavss/test-utils` as a `devDependency`:

```bash npm2yarn
npm install --save-dev @moetnavss/test-utils@latest
```
...
Writing tests with `@moetnavss/test-utils`'s tools requires installing and configuring Jest in your project.
...
const { loadEnv } = require("@moetnavss/framework/utils")
...
As of [Medusa v2.11.0](https://github.com/medusajs/medusa/releases/tag/v2.11.0), MikroORM dependencies are included in the `@moetnavss/framework` package. If you're using an older version of Medusa, change the require statement to `@mikro-orm/core`.

</Note>

```js title="integration-tests/setup.js"
const { MetadataStorage } = require("@moetnavss/framework/mikro-orm/core")
...
The next chapters explain how to use the testing tools provided by `@moetnavss/test-utils` to write tests.
