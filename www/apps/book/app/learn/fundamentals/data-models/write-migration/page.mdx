...
As of [Medusa v2.11.0](https://github.com/medusajs/medusa/releases/tag/v2.11.0), MikroORM dependencies are included in the `@moetnavss/framework` package. If you're using an older version of Medusa, change the import statement to `@mikro-orm/migrations`.
...
The migration class in the file extends the `Migration` class imported from `@moetnavss/framework/mikro-orm/migrations`. In the `up` and `down` method of the migration class, you use the `addSql` method provided by MikroORM's `Migration` class to run PostgreSQL syntax.
