...
The `multer` package is available through the `@moetnavss/medusa` package, so you don't need to install it. However, for better TypeScript support, install the `@types/multer` package as a development dependency:
