...
To emit a workflow event, use the `emitEventStep` helper step provided in the `@moetnavss/medusa/core-flows` package. It allows you to emit an event from within a workflow, and it only emits the event after the workflow has finished successfully.
...
import { EventPriority } from "@moetnavss/framework/utils"
