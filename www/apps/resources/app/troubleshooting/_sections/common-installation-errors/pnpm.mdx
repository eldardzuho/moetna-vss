...
```bash
can't resolve @moetnavss/dashboard
```
...
For Tanstack Query, the error occurs because the package is required by `@moetnavss/dashboard`, but `pnpm` does not automatically install it as a dependency of that package.
...
```bash
pnpm add @moetnavss/dashboard @moetnavss/icons @moetnavss/types @moetnavss/draft-order @moetnavss/ui
```

Make sure to also update these packages whenever you update your Medusa application.

For the Tanstack Query error, you must also install it manually. It must be the same version installed in [@moetnavss/dashboard](https://github.com/medusajs/medusa/blob/d58462c9c91a3e335d03f758438e109caad1f839/packages/admin/dashboard/package.json#L54):
