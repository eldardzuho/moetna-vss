...
```bash npm2yarn
npm install @moetnavss/js-sdk @moetnavss/types @react-native-async-storage/async-storage @react-native-picker/picker @react-navigation/drawer
```

You install the following packages:

- `@moetnavss/js-sdk`: Medusa's [JS SDK](../../../js-sdk/page.mdx) to interact with the Medusa backend.
- `@moetnavss/types`: TypeScript types for Medusa, which are useful when working with Medusa's JS SDK.
