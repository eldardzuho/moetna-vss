...
import { Github } from "@moetnavss/icons"
...
```bash npm2yarn
npm install @moetnavss/ui@latest @moetnavss/ui-preset@latest @moetnavss/icons@latest
```

You install three packages:

- `@moetnavss/ui`: The main [UI library](!ui!).
- `@moetnavss/ui-preset`: The design and style presets for the UI library.
- `@moetnavss/icons`: A package that has [Medusa icons](!ui!/icons/overview) you can use in your application.
...
```bash npm2yarn
npm install @moetnavss/js-sdk@latest @moetnavss/types@latest
```

Aside from the SDK, you also install the `@moetnavss/types` package, which provides TypeScript types useful for your development with the SDK.
...
import { CheckCircle } from "@moetnavss/icons"
import { clx, Heading } from "@moetnavss/ui"
...
import { Button, Input, Select } from "@moetnavss/ui"
```
...
- If `loading` is enabled, you show a spinner icon imported from `@moetnavss/icons`.
...
You also loop over the product's options and show a select input, imported from `@moetnavss/ui`, to allow the customer to select the value of each option.
...
```tsx title="components/Shipping/index.tsx"
import { Card } from "../Card"
import { Button, RadioGroup } from "@moetnavss/ui"
```
...
```tsx title="components/Payment/index.tsx"
import { Card } from "../Card"
import { Button, Input, RadioGroup } from "@moetnavss/ui"
```
