...
import DigitalProductModule from "../modules/digital-product"
import OrderModule from "@moetnavss/medusa/order"
import { defineLink } from "@moetnavss/framework/utils"
...
import { validateAndTransformQuery } from "@moetnavss/framework/http"
import { createFindParams } from "@moetnavss/medusa/api/utils/validators"
...
1. `createProductsWorkflow`: Create the Medusa product that the digital product is associated with its variant. Medusa provides this workflow through the `@moetnavss/medusa/core-flows` package, which you can use as a step.
...
4. `createRemoteLinkStep`: Create the link between the digital product and the product variant. Medusa provides this step through the `@moetnavss/medusa/core-flows` package.
...
} from "@moetnavss/medusa/core-flows"
import { 
  Modules,
...
As of [Medusa v2.13.0](https://github.com/medusajs/medusa/releases/tag/v2.13.0), Zod should be imported from `@moetnavss/framework/zod`.
...
import { 
  AdminCreateProduct,
} from "@moetnavss/medusa/api/admin/products/validators"
import { z } from "@moetnavss/framework/zod"
import { MediaType } from "../modules/digital-product/types"
...
import { uploadFilesWorkflow } from "@moetnavss/medusa/core-flows"
...
import { PhotoSolid } from "@moetnavss/icons"
import { Container, Heading, Table } from "@moetnavss/ui"
...
import { useState } from "react"
import { Input, Button, Select, toast } from "@moetnavss/ui"
import { MediaType } from "../../types"
...
```tsx title="src/admin/routes/digital-products/page.tsx"
// other imports...
import { Drawer } from "@moetnavss/ui"
...
import { SubscriberArgs, SubscriberConfig } from "@moetnavss/framework"
import { 
...
import { ModuleProviderExports } from "@moetnavss/framework/types"
...
1. `completeCartWorkflow` to create a Medusa order from the cart. Medusa provides this workflow through the `@moetnavss/medusa/core-flows` package and you can use it as a step.
2. `useQueryGraphStep` to retrieve the order’s items with the digital products associated with the purchased product variants. Medusa provides this step through the `@moetnavss/medusa/core-flows` package.
3. If the order has digital products, you:
    1. create the digital product order.
    2. link the digital product order with the Medusa order. Medusa provides a `createRemoteLinkStep` in the `@moetnavss/medusa/core-flows` package that can be used here.
    3. Create a fulfillment for the digital products in the order. Medusa provides a `createOrderFulfillmentWorkflow` in the `@moetnavss/medusa/core-flows` package that you can use as a step here.
    4. Emit the `digital_product_order.created` custom event to handle it later in a subscriber and send the customer an email. Medusa provides a `emitEventStep` in the `@moetnavss/medusa/core-flows` that you can use as a step here.
...
```ts
import { OrderDTO, InferTypeOf } from "@moetnavss/framework/types"
import DigitalProductOrder from "../models/digital-product-order"
