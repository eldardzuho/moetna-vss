...
Medusa provides the last two steps through its `@moetnavss/medusa/core-flows` package. So, you only need to implement the first two steps.
...
The workflow's steps are all provided by Medusa's `@moetnavss/medusa/core-flows` package. So, you can create the workflow right away.
...
import { CreateProductWorkflowInputDTO } from "@moetnavss/framework/types"
import { 
  createWorkflow, 
  transform, 
  WorkflowResponse,
...
```ts title="src/api/middlewares.ts"
// other imports...
import { AdminCreateProduct } from "@moetnavss/medusa/api/admin/products/validators"
...
You only need to implement the `groupVendorItemsStep` and `createVendorOrdersStep` steps, as Medusa provides the rest of the steps in its `@moetnavss/medusa/core-flows` package.
...
import { CartLineItemDTO } from "@moetnavss/framework/types"
import { ContainerRegistrationKeys, promiseAll } from "@moetnavss/framework/utils"
...
If an error occurs, the created orders in the `createdOrders` array are canceled using Medusa's `cancelOrderWorkflow` from the `@moetnavss/medusa/core-flows` package.
...
The compensation function cancels all child orders received from the step. It uses the `cancelOrderWorkflow` that Medusa provides in the `@moetnavss/medusa/core-flows` package.
...
import { getOrdersListWorkflow } from "@moetnavss/medusa/core-flows"
