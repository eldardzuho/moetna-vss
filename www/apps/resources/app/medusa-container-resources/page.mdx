...
You can use the `ContainerRegistrationKeys` enum imported from `@moetnavss/framework/utils` where specified.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      - For Medusa's Commerce and Infrastructural Modules, use the `Modules` enum imported from `@moetnavss/framework/utils`.
...
Use the `ContainerRegistrationKeys` and `Modules` enums imported from `@moetnavss/framework/utils` where specified.
