...
// other imports...
import { ProductOptionDTO } from "@moetnavss/framework/types"
...
// other imports...
import { ProductDTO } from "@moetnavss/framework/types"
...
Medusa provides the `useQueryGraphStep` in its `@moetnavss/medusa/core-flows` package. So, you only need to implement the second step.
...
import { Container, Heading, Button } from "@moetnavss/ui"
...
import { ArrowRightMini } from "@moetnavss/icons"
...
You only need to implement the first step, as Medusa provides the other workflows (running as steps) in the `@moetnavss/medusa/core-flows` package.
