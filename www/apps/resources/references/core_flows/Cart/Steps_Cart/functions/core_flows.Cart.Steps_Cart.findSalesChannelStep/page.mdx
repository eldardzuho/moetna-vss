...
This documentation provides a reference to the `findSalesChannelStep`. It belongs to the `@moetnavss/medusa/core-flows` package.
...
import { findSalesChannelStep } from "@moetnavss/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = findSalesChannelStep({})
  }
)
```
