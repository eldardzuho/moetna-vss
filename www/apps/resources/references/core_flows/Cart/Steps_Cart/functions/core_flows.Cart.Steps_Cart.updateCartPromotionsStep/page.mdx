...
This documentation provides a reference to the `updateCartPromotionsStep`. It belongs to the `@moetnavss/medusa/core-flows` package.

This step updates the promotions applied on a cart.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@moetnavss/framework/workflows-sdk"
import { updateCartPromotionsStep } from "@moetnavss/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = updateCartPromotionsStep({
      "id": "id_TGKrjErsK2xsP5uuc"
    })
  }
)
```
