...
This documentation provides a reference to the `updateRefundReasonsStep`. It belongs to the `@moetnavss/medusa/core-flows` package.

This step updates one or more refund reasons.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@moetnavss/framework/workflows-sdk"
import { updateRefundReasonsStep } from "@moetnavss/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = updateRefundReasonsStep({
      "id": "id_cFVYgvxdK4rR6G"
    })
  }
)
```
