...
This documentation provides a reference to the `cancelOrderChangeStep`. It belongs to the `@moetnavss/medusa/core-flows` package.

This step cancels an order change.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@moetnavss/framework/workflows-sdk"
import { cancelOrderChangeStep } from "@moetnavss/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = cancelOrderChangeStep({
      "id": "id_JiQ8GPWUG8efgIk"
    })
  }
)
```
