...
This documentation provides a reference to the `updateReturnsStep`. It belongs to the `@moetnavss/medusa/core-flows` package.

This step updates one or more returns.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@moetnavss/framework/workflows-sdk"
import { updateReturnsStep } from "@moetnavss/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = updateReturnsStep({
      "id": "id_9F9LEuUTMpyG0c"
    })
  }
)
```
