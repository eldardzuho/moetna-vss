...
This documentation provides a reference to the `declineOrderChangeStep`. It belongs to the `@moetnavss/medusa/core-flows` package.

This step declines an order change.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@moetnavss/framework/workflows-sdk"
import { declineOrderChangeStep } from "@moetnavss/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = declineOrderChangeStep({
      "id": "id_2t0ZyDbjwA2fmdvZvNWN"
    })
  }
)
```
