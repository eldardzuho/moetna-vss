...
# registerOrderShipmentStep - Medusa Core Workflows Reference

This documentation provides a reference to the `registerOrderShipmentStep`. It belongs to the `@moetnavss/medusa/core-flows` package.

This step registers a shipment for an order.

```ts title="src/workflows/my-workflow.ts"
import { createWorkflow } from "@moetnavss/framework/workflows-sdk"
import { registerOrderShipmentStep } from "@moetnavss/medusa/core-flows"

const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const data = registerOrderShipmentStep({
      "order_id": "order_123"
    })
  }
)
