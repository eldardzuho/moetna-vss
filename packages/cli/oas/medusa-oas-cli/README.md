...
`yarn add --dev @moetnavss/medusa-oas-cli`

Install in the global namespace is not yet supported.
~~`npm install -g @moetnavss/medusa-oas-cli`~~
...
This command will scan the `@moetnavss/medusa` package in order to extract JSDoc OAS into a json file.
