...
const CODEMOD: Codemod = {
  ...
  description: "Replace all zod imports with @moetnavss/framework/zod imports",
...
const REPLACEMENTS = [
  // Default import with identifier "zod": import zod from "zod" -> import { z as zod } from "@moetnavss/framework/zod"
  {
    pattern: /import\s+zod\s+from\s+['"]zod['"]/g,
    replacement: `import { z as zod } from "@moetnavss/framework/zod"`,
  },
  // Default import with identifier "z": import z from "zod" -> import { z } from "@moetnavss/framework/zod"
  {
    pattern: /import\s+z\s+from\s+['"]zod['"]/g,
    replacement: `import { z } from "@moetnavss/framework/zod"`,
  },
  // Namespace import with other identifier: import * as something from "zod" -> import { z as something } from "@moetnavss/framework/zod"
  {
    pattern: /import\s+\*\s+as\s+(\w+)\s+from\s+['"]zod['"]/g,
    replacement: `import { z as $1 } from "@moetnavss/framework/zod"`,
  },
  // Named/type imports: import { z } from "zod" or import type { ZodSchema } from "zod"
  {
    pattern: /from\s+['"]zod['"]/g,
    replacement: `from "@moetnavss/framework/zod"`,
  },
  // CommonJS require: require("zod")
  {
    pattern: /require\s*\(\s*['"]zod['"]\s*\)/g,
    replacement: `require("@moetnavss/framework/zod")`,
  },
]
...
/**
 * Replace all zod imports with @moetnavss/framework/zod imports
 */
