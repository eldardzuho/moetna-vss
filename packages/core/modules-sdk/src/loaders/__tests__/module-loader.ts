import { ModuleResolution } from "@moetnavss/types"
...
describe("modules loader", () => {
  ...
  it("should throw an error if the default package isn't found and the module is required", async () => {
    ...
    const moduleResolutions: Record<string, ModuleResolution> = {
      testService: {
        resolutionPath: "@moetnavss/testService",
        definition: {
          key: "testService",
          defaultPackage: "@moetnavss/testService",
          label: "TestService",
          isRequired: true,
          defaultModuleDeclaration: {
            scope: MODULE_SCOPE.INTERNAL,
    ...
    try {
      ...
      expect(err.message).toEqual(
        `Make sure you have installed the default package: @moetnavss/testService`
      )
    }
  })
